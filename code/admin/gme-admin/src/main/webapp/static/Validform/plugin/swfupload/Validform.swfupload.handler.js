	...
	fileQueueError:function(file, errorCode, message){
		try {
			...
			case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
				...
				this.debug("错误提示: 文件太大, 文件名:" + file.name + ", 文件大小: " + file.size + ", 消息: " + message);
				return;
			case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
				this.customSettings.showmsg("文件为空,请重新选择.",3);
				this.debug("错误提示: 文件为空, 文件名: " + file.name + ", 文件大小: " + file.size + ", 消息: " + message);
				return;
			case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE:
				this.customSettings.showmsg("不允许上传该类型文件.",3);
				this.debug("错误提示: 非法文件类型, 文件名: " + file.name + ", 文件大小: " + file.size + ", 消息: " + message);
				return;
			default:
				swfu.customSettings.showmsg("上传失败,请重试.",3);
				this.debug("错误提示: " + errorCode + ", 文件名: " + file.name + ", 文件大小: " + file.size + ", 消息: " + message);
				return;
			}
		} catch (e) {}	
	},
