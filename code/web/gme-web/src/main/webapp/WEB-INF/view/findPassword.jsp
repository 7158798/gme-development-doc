...
<html>
    ...
    <title>找回密码 - Gate.io - The Gate of Blockchain Assets Exchange</title>
    ...
    <style>.icon-48,.icon-32,.icon-16{background-image:url("images/coins48.png?v=")}</style>
    <link href="css/coins_16.css?v=" rel="stylesheet" type="text/css">
    <link href="css/coins_32.css?v=" rel="stylesheet" type="text/css">
    <link href="css/coins_48.css?v=" rel="stylesheet" type="text/css">
...
<div class="header clearfix">
    <div class="top-up">
        <div class="top-con">
            <ul class="topprice">
				<li> BTC/CNY : ￥ <span class="topnum"></span><i class="icon-arrow-"></i> </li>
                <li> ETH/CNY : ￥ <span class="topnum"></span><i class="icon-arrow-"></i> </li>
                <li> LTC/CNY : ￥ <span class="topnum"></span><i class="icon-arrow-"></i> </li>
                <li> QTUM/CNY : ￥ <span class="topnum"></span><i class="icon-arrow-"></i> </li>
            </ul>
            <div class="qqtel">
...
<style>
	...
	.reset-psw-con{min-height: calc(100% - 537px); border-top: 1px solid #ddd}
	.dark-body .reset-psw-con{min-height: calc(100% - 568px); border-color: #343a48}
</style>
<div class="left_con_login reset-psw-con" style="padding-top: 30px;">
	...
	<div class="mairu">
		<div class="m_title"><h4>重置登录密码</h4></div>
		<div class="m_con_buy leftregi">
			<form name="login" method="post" action="https://gateio.io/resetpw" >		
			<dl style="text-align:left; padding-top:20px">
				<dt><label for="email">请输入您的 邮箱/手机号</label></dt>
				<dd><input type="text" name="email" id="email" value=""/></dd>
				<dt><label for="captcha">验证码</label><dt>
				<dd><input type="text" name="captcha" id="captcha"/><img id="respCaptcha" src="/captcha" alt="Captcha" title="看不清？换一张" onclick="document.getElementById('respCaptcha').src = '/captcha?' + Math.random(); return false"/></dd>
				<div style="clear:both"></div>
				<dd><input type="submit" value="提交" class="sub-btn" style="height:40px; margin-bottom:20px"></dd>
                <dd>
                                        </dd>
			</dl>
			</form>	
...
$(document).ready(function (e) {
	$('#smsbtn').click(function() {
        ...
        var resetTimer = function()
        {
			...
			$.ajax({type:"post",url:"/sms/" + smsid, data:req,
				...
				error : function(){
					timer.play();
					noty({text: '网络错误!', type: "warning",layout: "bottomLeft", timeout: 10000});
				},
				complete : function(){
				}
			});
    ...
    $('#voicebtn').click(function() {
        ...
        var resetTimer = function()
        {
			...
			$.ajax({type:"post",url:"/voiceverify/" + smsid, data:req,
				success : function(data, textStatus){
					var json_res = data;
					if ( json_res && json_res.result ) {
						self.text(orignialtext + ' (' + currentTime + ')');
						...
						$('#smscode', form).prop('disabled', false);
						 ...
						 msg = lang == "en" ? "Send voice faild: "+json_res.msg+", please check your phone number at <a href='/myaccount/sms_setup'>this link</a>." : "电话语音拨打失败："+json_res.msg+"，请到<a href='/myaccount/sms_setup'>这里检查手机号码设置</a>.";					
						 timer.play();
					}
					noty({text: msg, type: "warning",layout: "bottomLeft", timeout: 10000});
				},
...
<div class="footer">
    ...
    <div class="fnav">
        ...
        <div class="tail">
            ...
            <div class="vol-all">
                <span>成交量:</span>
				<span> USDT : $<span id="usdtAll"></span> </span>
                <span> BTC : ฿<span id="btcAll"></span> </span>
                <span> LTC : Ł<span id="ltcAll"></span> </span>
                <span> ETH : E<span id="ethAll"></span> </span>
            </div>
            <span id="runTime">
                <!--<a href="#">gate.io</a> is powered by <a href="#">gate.io Dev Team</a>-->
                <span class="admin-view"> Elapsed:0.081ms &nbsp; a/e/r </span>
            </span>
        </div>
    </div>
</div>
...
<script>
    ...
    $("#usdtAll").text(toThousands());
    $("#btcAll").text(toThousands());
    $("#ltcAll").text(toThousands());
    $("#ethAll").text(toThousands());
    is_login='' != '';
</script>
